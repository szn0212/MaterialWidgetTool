package com.study.library.utils;
 ...
 * @author suzhuning
 * @date 2016/12/5.
 * Description:状态栏、通知栏等相关工具类
 *      setColor            设置状态栏颜色
 *      setColorForSwipeBack    为滑动返回界面设置状态栏颜色
 *      setColorNoTranslucent   设置状态栏纯色，不加透明效果
 *      setColorDiff            设置状态栏颜色（5.0以下无半透明效果，不建议使用）
 *      setTranslucent          使状态栏半透明
 *      setTranslucentForCoordinatorLayout      针对根布局是CoordinatorLayout, 使状态栏半透明
 *      setTransparent          设置状态栏全透明
 *      setTranslucentDiff      使状态栏透明(5.0以上半透明效果,不建议使用)
 *      setColorForDrawerLayout 为DrawerLayout 布局设置状态栏变色
 *      setColorNoTranslucentForDrawerLayout        为DrawerLayout布局设置状态栏颜色,纯色
 *      setColorForDrawerLayout 为DrawerLayout布局设置状态栏变色
 *      setColorForDrawerLayoutDiff     为DrawerLayout布局设置状态栏变色(5.0以下无半透明效果,不建议使用)
 *      setTranslucentForDrawerLayout   为DrawerLayout 布局设置状态栏透明
 *      setTranslucentForDrawerLayoutDiff       为DrawerLayout布局设置状态栏透明(5.0以上半透明效果,不建议使用)
 *      setTransparentForImageView      为头部是ImageView的界面设置状态栏全透明
 *      setTranslucentForImageView      为头部是ImageView的界面设置状态栏透明(使用默认透明度)
 *      setTranslucentForImageViewInFragment    为fragment头部是ImageView的设置状态栏透明
 *      setTransparentForImageViewInFragment    为fragment头部是ImageView的设置状态栏透明
