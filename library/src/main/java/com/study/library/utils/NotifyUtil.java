 ...
 * @date 2016/12/7.
 * Description:通知相关工具类
 *      NotifyUtil          构造方法，通知id
 *      setCompatBuilder    设置在顶部通知栏中的各种信息
 *      setBuilder          设置builder的信息，在用大文本时会用到这个
 *      notify_normal_singline      普通通知
 *      notify_mailbox              进行多项设置的通知(在小米上似乎不能设置大图标，系统默认大图标为应用图标)
 *      notify_customview           自定义视图的通知
 *      notify_normail_moreline     可以容纳多行提示文本的通知信息 (因为在高版本的系统中才支持，所以要进行判断)
 *      notify_progress             有进度条的通知，可以设置为模糊进度或者精确进度
 *      notify_bigPic               容纳大图片的通知
 *      notify_button               里面有两个按钮的通知
 *      notify_HeadUp               HeadUp样式通知
 *      sent                发送通知
 *      clear               取消通知
 */
