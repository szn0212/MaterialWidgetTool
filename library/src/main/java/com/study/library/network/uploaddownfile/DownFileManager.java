...
public class DownFileManager implements DownSingleFileListener {
    ...
    private static String TAG = DownFileManager.class.getSimpleName();
    ...
    public synchronized void downSingleFileProgress(FileBean fileBean) {
        ...
        Log.i(TAG, "--total count" + totalCount + "---success count" + successCount + "---fail count" + failCount + "sending counet" + sendingCount + "---progress" + progress + "--file progress" + fileBean.progress);
    ...
    public void downSingleFileComplete(FileBean fileBean) {
        Log.i(TAG, "--downSingleFileComplete file url" + fileBean.fileUrl + "file status:" + fileBean.status);
        if (fileBean.status == FileStatus.DOWN_SUCCESS) {
            ...
            //上传失败
            Log.i(TAG, "---fail error " + fileBean.error + "error count" + fileBean.failCount);
            boolean remove = mSendingFileList.remove(fileBean);
            fileBean.failCount++;
        ...
        int failCount = mFailFileList.size();
        int sendingCount = mSendingFileList.size();
        Log.i(TAG, "---total count" + totalCount + "---success count" + successCount + "---fail count" + failCount + "sending counet" + sendingCount);
                ...
                if (mDownFileList.size() > 0) {
                    ...
                    if (mFailFileList.size() > 0) {
                        ...
                        if (failFileBean.failCount <= MAX_FAIL_COUNT) {
                            Log.i(TAG, "--- resend error" + fileBean.error + "---resend count" + fileBean.failCount + "---fail file" + fileBean.fileUrl);
                            ...
                            //大于最大失败次数,直接返回
                            Log.i(TAG, "---fail error" + fileBean.error + "---fail url" + fileBean.fileUrl);
                            if (mDownFileListener != null) {
